...
P3VALS = list[SQLD] | TVals | TVals2 | TVals3 | None | list[str]
...
class SQL_Executor:
    ...
    @staticmethod
    def execute_select(cursor: sqlite3.Cursor, table: str, column: str, value: P3VALS = None):
        try:
            if value is None:
                cursor.execute(f"SELECT * FROM {table} WHERE {column} IS NULL")
            elif isinstance(value, list):
                cursor.execute(f"SELECT * FROM {table} WHERE {column} IN ({', '.join(['?'] * len(value))})", value)
